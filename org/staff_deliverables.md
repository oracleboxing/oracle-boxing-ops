# Staff Deliverables

**Version:** 2.1
**Last Updated:** 2025-12-14
**Purpose:** Single source of truth mapping each staff member to their roles and deliverables.

---

## Summary Table

| Person | Role(s) | Primary Department(s) |
|--------|---------|----------------------|
| Jordan | Managing Director, Growth Operator, Operations Manager | Executive, Growth, Ops |
| Oliver | Content Creator, Coach–Educator | Content, Product & Coaching |
| Toni | Coach–Educator, Sales Rep | Product & Coaching, Growth |
| Charlie | Coach–Educator | Product & Coaching |
| G | Content Producer (Editor), Content Manager (Publishing & Performance) [interim] | Content |

---

## Jordan

**Roles:** Managing Director (Founding Partner); Growth Operator; Operations Manager

### Executive & Strategy
| Deliverable | Cadence |
| --- | --- |
| Conduct quarterly budget allocation review | Quarterly |
| Set quarterly focus and key bets | Quarterly |
| Update ops dashboard with weekly priorities and status | Weekly |
| Conduct weekly planning session | Weekly |
| Make hiring and resource allocation decisions | As needed |
| Conduct performance reviews | Quarterly |
| Update organisational blueprint as needed | As needed |

### Growth (Marketing & Sales)
| Deliverable | Cadence |
| --- | --- |
| Create campaign overview (theme, timing, duration, rationale, unique angle) | Per campaign |
| Write copywriting for the landing page | Per campaign |
| Script and produce video sales letter (VSL) | Per campaign |
| Build front-end of the landing page | Per campaign |
| Build back-end of the landing page (forms, integrations, tracking) | Per campaign |
| Define pricing and available packages | Per campaign |
| Write email campaign sequence | Per campaign |
| Script YouTube mid-roll advert | Per campaign |
| Liaise with Content to produce the mid-roll advert | Per campaign |
| Update YouTube video descriptions with campaign links | During campaign |
| Create organic Instagram assets (reels, carousels, stories) | Per campaign |
| Create paid Meta ads marketing assets | Per campaign |
| Schedule and launch Meta ads campaign | Per campaign |
| Monitor campaign performance (Facebook ad group) | During campaign |
| Post-campaign debrief (what went well, what didn't) | Per campaign |
| Remove/close special landing page | End of campaign |
| Remove YouTube mid-roll ads if necessary | End of campaign |
| Upsell challenge customers to continuity via mid-challenge check-in call | During challenges |

### Operations & Finance
| Deliverable | Cadence |
| --- | --- |
| Answer customer emails | Daily |
| Process refunds where possible | As needed |
| Track profit and loss | Monthly |
| Pay contractor wages and commissions | Monthly |
| Ensure all automations are operating functionally | Weekly review |
| Ensure website is operating functionally | Weekly checks |
| Troubleshoot any software errors and bugs | As issues arise |
| File quarterly VAT return | Quarterly |
| File annual corporation tax return | Annually |
| File annual self assessments | Annually |
| File monthly PAYE for directors | Monthly |
| Look for any inefficiencies to remove or automate | Ongoing |
| Engage Dev/Tech contractor via SOW for projects | As needed |

### Compensation
**Model:** Flat + Tiered Commission + Hourly | [Full details](compensation/COMPENSATION_SYSTEM.md#jordan--managing-director--growth-operator--operations-manager)

| Component | Amount | Type |
| --- | --- | --- |
| Growth Operator base | £1,000/month | Flat |
| Campaign commission | Tiered 10–20% (£3k cap) | Per campaign |
| Challenge upsell calls | 10% of collected revenue | Commission |
| Ops & Finance work | £20/hour | Hourly (pre-approved) |
| MD constraint bounties | £1,000–£2,500 each | Success-only ([details](roles/managing_director.md#success-only-compensation-model)) |
| MD run-rate milestones | £2,000–£5,000 | One-time per threshold |

---

## Oliver

**Roles:** Content Creator; Coach–Educator

### Content
| Deliverable | Cadence |
| --- | --- |
| Script 8 YouTube videos per month (2 per week) | Monthly |
| Film 8 YouTube videos per month (2 per week) | Monthly |
| Film weekly vlog updates | Weekly |
| Upload raw YouTube videos to Dropbox and link in content schedule | Per video |
| Weekly editing + presentation review (Saturday morning) | Weekly (Saturday) |

### Product & Coaching
| Deliverable | Cadence |
| --- | --- |
| Create or update / maintain high quality course material | Ongoing |
| Host group coaching calls (Wed 9am, Thu 9am, Fri 9am UK) | Weekly (3 sessions) |
| Plan coaching calls at least 10 minutes in advance | Per session |
| Publish coaching call recordings within 24 hours | Per session |
| Deliver a 45-minute 1-on-1 coaching session to top 3 on 30-day leaderboard | Monthly |
| Provide a "satisfactory" level of community activity | Ongoing |

### Compensation
**Model:** Flat + Unit + Performance | [Full details](compensation/COMPENSATION_SYSTEM.md#oliver--content-creator--coachecator)

| Component | Amount | Type |
| --- | --- | --- |
| Content Creator base | £1,500/month | Flat |
| AVD% bonus (40/45/50%) | £300/£600/£900 | Performance |
| Group coaching call | £30/session | Unit |
| 1-on-1 coaching session | £30/session | Unit |
| Course creation | £20/hour | Hourly (pre-approved) |

---

## Toni

**Roles:** Coach–Educator; Sales Rep

### Product & Coaching
| Deliverable | Cadence |
| --- | --- |
| Host group coaching calls (Mon 8pm, Tue 9pm, Sat 5pm, Sun 12pm UK) | Weekly (4 sessions) |
| Plan coaching calls at least 10 minutes in advance | Per session |
| Publish coaching call recordings within 24 hours | Per session |
| Deliver a 45-minute 1-on-1 coaching session to top 3 on 7-day leaderboard | Weekly |
| Deliver a 45-minute 1-on-1 coaching call for paid coaching packages | 1-2 per week |
| Deliver a 15-minute onboarding call to every new member | As members join |
| Provide a "satisfactory" level of community activity | Ongoing |

### Growth (Sales)
| Deliverable | Cadence |
| --- | --- |
| Identify potential customers interested in 1-1 coaching | Ongoing |
| Handle pre-purchase enquiries and answer questions | As enquiries come in |
| Explain the 1-1 coaching packages and prices to prospects | As enquiries come in |
| Collect payment for 1-on-1 coaching package and update tracker | Per sale |

### Compensation
**Model:** Commission + Floor | [Full details](compensation/COMPENSATION_SYSTEM.md#toni--coachecator--sales-rep)

| Component | Amount | Type |
| --- | --- | --- |
| 1-on-1 coaching sales | 60% of sales | Commission |
| Monthly floor guarantee | £1,000 | Floor |

*Commission calculated on collected revenue, net of refunds within agreed refund window.*

---

## Charlie

**Roles:** Coach–Educator

### Product & Coaching
| Deliverable | Cadence |
| --- | --- |
| Host group coaching calls (Wed 11pm, Sat 12am UK) | Weekly (2 sessions) |
| Plan coaching calls at least 10 minutes in advance | Per session |
| Publish coaching call recordings within 24 hours | Per session |
| Deliver a 45-minute 1-on-1 coaching session to members ranked 4-10 on 7-day leaderboard | Weekly |
| Leave weekly video feedback on qualifying member posts (key takeaways, actionables, <5 mins) | Weekly |
| Provide a "satisfactory" level of community activity | Ongoing |

### Compensation
**Model:** Flat | [Full details](compensation/COMPENSATION_SYSTEM.md#charlie--coachecator)

| Component | Amount | Type |
| --- | --- | --- |
| Base | Existing base (unchanged) | Flat |

---

## G

**Roles:** Content Producer (Editor); Content Manager (Publishing & Performance) [interim]

### Content – Editing
| Deliverable | Cadence |
| --- | --- |
| Edit 2 YouTube videos per week for high retention, views, and engagement | Weekly |
| Edit weekly vlog | Weekly |
| Repurpose long-form footage into shorts (2 per week minimum) | Weekly |
| Create additional short-form content as needed (1-2 per week) | Weekly |
| Weekly editing + presentation review (Saturday morning) | Weekly (Saturday) |

### Content – Publishing & Performance [interim]
| Deliverable | Cadence |
| --- | --- |
| Plan a high-CTR worthy title for each YouTube video (8 per month) | Per video |
| Write SEO-optimised descriptions | Per video |
| Coordinate Fiverr thumbnails (3 variants for ABC testing) | Per video |
| Schedule video with appropriate end cards and mid cards | Per video |
| Manage publishing schedule and calendar | Weekly |
| Track video performance and report insights | Weekly |

### Compensation
**Model:** Flat + Unit + Performance | [Full details](compensation/COMPENSATION_SYSTEM.md#g--content-producer-editor--content-manager-interim)

| Component | Amount | Type |
| --- | --- | --- |
| Base retainer | £300/month | Flat |
| Short-form videos | £8/short | Unit |
| AVD% bonus (35/40/45/50%) | £200/£400/£650/£900 | Performance |
| Meta interactions bonus (50k/75k/100k/150k) | £100/£200/£350/£500 | Performance |

*AVD% bonus conditions: All scheduled videos ship on time + weekly insights report delivered.*

---

## Notes

- **Jordan** currently holds 3 roles; mid-term goal is to delegate Growth Operator and Operations Manager
- **Oliver** splits time between Content Creator and Coach–Educator
- **Toni** handles both coaching and sales execution
- **G** holds Content Producer/Editor and Content Manager roles [interim]
- **Thumbnails** outsourced to Fiverr (coordinated by G)
- **Community Steward** is the immediate priority hire to support member engagement

---

## Related Documents

- [Start Here](START_HERE.md) – Navigation hub
- [Organisational Blueprint](organisational_blueprint.md) – Department structure
- [Roles Index](roles/README.md) – Role definitions
- [Compensation System](compensation/COMPENSATION_SYSTEM.md) – Pay structures and formulas
