# Coach–Educator

**Version:** 1.0
**Last Updated:** 2025-12-13
**Purpose:** Define the Coach–Educator role scope, decision rights, and success measures for Oracle Boxing.

---

## Role Summary

| Attribute | Value |
| --- | --- |
| **Current Holders** | Oliver, Toni, Charlie |
| **Primary Department** | Product & Coaching |
| **Reports To** | Managing Director (Founding Partner) |
| **Collaborates With** | Other Coach–Educators, Community & Retention |

---

## Scope

Deliver boxing coaching through group sessions, 1-1 coaching, video feedback, and community engagement. Drive student results through consistent, high-quality coaching and accountability. Different team members hold this role with varying load allocations.

---

## Current Team & Load Allocation

| Person | Primary Focus | Weekly Load |
| --- | --- | --- |
| **Oliver** | Course creation, group coaching, monthly 1-1s | 3 group sessions + 3 monthly 1-1s |
| **Toni** | Group coaching, weekly 1-1s, paid packages, onboarding, sales | 4 group sessions + 3 weekly 1-1s + 1-2 paid |
| **Charlie** | Group coaching, weekly 1-1s, video feedback | 2 group sessions + 7 weekly 1-1s + feedback |

---

## Key Responsibilities

### Group Coaching Sessions
- [ ] Punctually host assigned group coaching calls
- [ ] Plan the coaching call at least 10 minutes in advance
- [ ] Deliver engaging, educational coaching content
- [ ] Publish the recording of the call within 24 hours

### 1-1 Coaching
- [ ] Deliver 45-minute 1-1 sessions to leaderboard winners (weekly/monthly)
- [ ] Deliver 45-minute 1-1 sessions to paid package customers
- [ ] Prepare personalised feedback for each session
- [ ] Track session completion and member progress

### Video Feedback (Charlie)
- [ ] Leave weekly video feedback on qualifying member posts
- [ ] Follow criteria: key takeaways, actionables, below 5 minutes
- [ ] Clear feedback queue weekly

### Community Engagement
- [ ] Maintain active presence in community (comments, likes, encouragement)
- [ ] Respond to member questions and posts
- [ ] Model positive community behaviour

### Onboarding (Toni)
- [ ] Deliver 15-minute onboarding call to every new member
- [ ] Set expectations and help members get started
- [ ] Answer initial questions and provide guidance

### Course Creation (Oliver)
- [ ] Create and update high-quality course material
- [ ] Ensure course content is current and effective
- [ ] Incorporate feedback from coaching sessions

---

## Weekly Coaching Schedule (UK Time)

| Day | Time | Host |
| --- | --- | --- |
| Monday | 8pm | Toni |
| Tuesday | 9pm | Toni |
| Wednesday | 9am | Oliver |
| Wednesday | 11pm | Charlie |
| Thursday | 9am | Oliver |
| Friday | 9am | Oliver |
| Saturday | 12am | Charlie |
| Saturday | 5pm | Toni |
| Sunday | 12pm | Toni |

---

## Decision Rights

### Decides Without Asking
- Session content and teaching approach
- Personal coaching style within brand guidelines
- Scheduling of 1-1 sessions within allocated slots
- Feedback approach for video reviews

### Must Escalate
- Schedule changes affecting coverage (coordinate with team)
- Member concerns or complaints (Managing Director)
- Capacity constraints (Managing Director)
- Course content strategy changes (Managing Director / Oliver)

---

## KPIs / Success Measures

- **Session delivery:** 100% of scheduled sessions delivered
- **Punctuality:** Sessions start on time
- **Recording turnaround:** Posted within 24 hours
- **1-1 completion:** All leaderboard/paid sessions delivered
- **Video feedback:** Queue cleared weekly (Charlie)
- **Onboarding completion:** 100% of new members receive call (Toni)
- **Community presence:** Active engagement maintained

---

## Interfaces

| Who | Collaboration Purpose | Frequency |
| --- | --- | --- |
| Other Coach–Educators | Schedule coordination, coverage, best practices | Weekly |
| Community & Retention | Member engagement signals, at-risk members | As needed |
| Content | Coaching insights for video topics | As needed |
| Growth | Success stories, upsell opportunities | As needed |

---

## Compensation

Compensation varies by holder. See [Compensation System](../compensation/COMPENSATION_SYSTEM.md) for full details.

| Person | Model | Key Components |
| --- | --- | --- |
| Oliver | Flat + Unit | £30/group session, £30/1-1 session |
| Toni | Commission + Floor | 60% of 1-1 sales, £1,000 floor |
| Charlie | Flat | £800/month |

---

## Notes

- Coach–Educator is a unified role; the old titles (Community Coach, 1-1 Coach, Course Creator) are deprecated
- Load allocation determines specific deliverables per person
- Oliver also holds Content Creator role
- Toni also holds Sales Rep role (for 1-1 coaching sales)

---

## Related Documents

- [Product & Coaching Department](../departments/product_and_coaching.md)
- [Content Creator Role](content_creator.md) (Oliver)
- [Staff Deliverables](../staff_deliverables.md)
