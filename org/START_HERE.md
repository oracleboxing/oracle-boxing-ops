# Oracle Boxing – Start Here

**Version:** 1.0
**Last Updated:** 2025-12-13
**Purpose:** Quick navigation hub for the Oracle Boxing Operating System.

---

## Quick Links

| Document | Purpose |
|----------|---------|
| [Organisational Blueprint](organisational_blueprint.md) | Core company model – departments, outcomes, hiring roadmap |
| [Ops Dashboard](ops_dashboard.md) | Weekly operational snapshot – priorities, owners, status |
| [Staff Deliverables](staff_deliverables.md) | Person → role(s) → deliverables mapping (single source of truth) |
| [Compensation System](compensation/COMPENSATION_SYSTEM.md) | Pay structures, bonus tiers, and payout cadence |
| [MD Success-Only Model](roles/managing_director.md#success-only-compensation-model) | Constraint bounties and run-rate milestones for Managing Director |

---

## Departments

| Department | File | Outcome |
|------------|------|---------|
| Executive & Strategy | [View](departments/executive_and_strategy.md) | Direction, priorities, resourcing, culture |
| Product & Coaching | [View](departments/product_and_coaching.md) | Student results through curriculum and live coaching |
| Community & Retention | [View](departments/community_and_retention.md) | High retention, community health, accountability |
| Content | [View](departments/content.md) | Consistent high-quality content pipeline that feeds growth |
| Growth (Marketing & Sales) | [View](departments/growth_marketing_and_sales.md) | Predictable acquisition + conversion across offers |
| Operations & Finance | [View](departments/operations_and_finance.md) | Reliability, support, payments, compliance |

---

## Roles

| Role | File | Current Holder(s) |
|------|------|-------------------|
| Managing Director (Founding Partner) | [View](roles/managing_director.md) | Jordan |
| Content Creator | [View](roles/content_creator.md) | Oliver |
| Content Producer (Editor) | [View](roles/content_producer_editor.md) | G |
| Content Manager (Publishing & Performance) | [View](roles/content_manager.md) | G [interim] |
| Coach–Educator | [View](roles/coach_educator.md) | Oliver, Toni, Charlie |
| Community Steward | [View](roles/community_steward.md) | Future hire |
| Growth Operator | [View](roles/growth_operator.md) | Jordan (→ future hire) |
| Operations Manager | [View](roles/operations_manager.md) | Jordan (→ future hire) |
| Dev / Tech (Contract) | [View](roles/dev_tech_contract.md) | On-demand |

---

## Other Resources

- [Roles Index & Guidelines](roles/README.md)
- [Responsibilities Index](responsibilities/README.md)

---

## How to Use This Repo

1. **New team member?** Start with the [Organisational Blueprint](organisational_blueprint.md) to understand the company model.
2. **Looking for your deliverables?** Check [Staff Deliverables](staff_deliverables.md).
3. **Need weekly priorities?** See [Ops Dashboard](ops_dashboard.md).
4. **Deep-dive into a department?** Use the department links above.
5. **Understand a specific role?** Use the roles links above.

---

## Update Cadence

- **Quarterly:** Budget allocation review + key bets
- **Weekly:** Ops dashboard update
- **As needed:** Org blueprint, role, and deliverable updates when structure changes
